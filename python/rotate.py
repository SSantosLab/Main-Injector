...
# the accuracy isn't good enough?
    ...
    try:
        index = np.nonzero(ra < 0)
        ra[index] = ra[index]+360
        index = np.nonzero(ra >= 360)
        ra[index] = ra[index]-360
        index = np.nonzero(dec > 90)
        dec[index] = 90-(dec[index]-90)
        index = np.nonzero(dec < -90)
        dec[index] = -90+(dec[index]+90)
    except:
        pass
...
def rotateAboutXaxis(x, y, z, alpha, verbose=0):
    if verbose:
        print("\t x axis rotation of ", alpha, "given  ", x[0], y[0], z[0])
...
def rotateAboutYaxis(x, y, z, alpha, verbose=0):
    if verbose:
        print("\t y axis rotation of ", alpha, "given  ", x[0], y[0], z[0])
    alpha = alpha*2*np.pi/360.
    # correct
...
def rotateAboutZaxis(x, y, z, alpha, verbose=0):
    if verbose:
        print("\t z axis rotation of ", alpha, "given  ", x[0], y[0], z[0])
    alpha = alpha*2*np.pi/360.
...
def test_euler(raCen, decCen):
    ...
    ra, dec, r = cartesian2spherical(x, y, z)
    ra = np.array(ra)
    dec = np.array(dec)
