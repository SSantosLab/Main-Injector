    ...
    def __init__(self,
        ...
        if use_map:
            ...
            if not os.path.isdir(lowres_path):
                os.mkdirs(lowres_path,exist_ok=True)
            
            if not os.path.isdir(weights_path):
                os.mkdirs(weights_path,exist_ok=True)
        ...
        if use_map != "":
            ...
            pb_check=pb_check[distmu_check > 0]
            distsigma_check=distsigma_check[distmu_check > 0]
            distmu_check=distmu_check[distmu_check > 0]
            
            pb_check=pb_check[np.logical_not(distmu_check > 10000)]
            distsigma_check=distsigma_check[np.logical_not(distmu_check > 10000)]
            distmu_check=distmu_check[np.logical_not(distmu_check > 10000)]
            
            pb_check=pb_check[np.logical_not(distsigma_check > 10000)]
            distmu_check=distmu_check[np.logical_not(distsigma_check > 10000)]
            distsigma_check=distsigma_check[np.logical_not(distsigma_check > 10000)]
            ...
            if reduce_mapresolution == True:
                ...
                if np.isinf(distmu).any():
                    ...
                    distmu[np.isinf(distmu)] = 100000  # 10**25
                else:
...
if __name__ == '__main__':
    ...
    if 1 == 1:
        ...
        if m_exp_run == True:
            ...
            if (sky_area >= 300.0) and (sky_area <= 1000.0):
                area_covered = [0.8, 0.8, 0.8]
                area_covered_deep = [0.7, 0.6, 0.5]

            if sky_area > 1000.0:
                area_covered = [0.7, 0.7, 0.7]
                area_covered_deep = [0.6, 0.5, 0.3]
        else:
