...
from main_injector.utils import hp2np, decam2hp, jsonMaker, obsSlots
from main_injector.gwhelper import gwwide
from main_injector.kasen import kasen_modelspace
from main_injector.skymaps import mags, mapsAtTimeT, inside_footprint, mcbryde
from main_injector.utils import cumul, des_optimization
from main_injector.plots import mcplot, plotMapAndHex
...
def make_maps(gw_map_trigger, gw_map_strategy, gw_map_control, gw_map_results):
    ...
    if snarf_mi_maps:
        ...
        os.system(
            "cp {}/*probabilityTimeCache*txt {}/".format(mi_map_dir, data_dir))
        os.system("cp {}/*-hexVals.txt {}/".format(mi_map_dir, data_dir))
        os.system(
            "cp {}/*-hexVals-cutOverlappingProb.txt {}/".format(mi_map_dir, data_dir))
        return
...
# we need to understand the night, how long it is, etc
    ...
    try:  # ag  test to see if we can get past this spot. sept 6 2022
        apparent_mag = kasen_modelspace.run_ap_mag_for_kasen_models(
            ...
            fast=False)
        print("\t\t at a time halfway through night, {:.2f} days after merger:".format(
            midnight_since_burst/24.))
        print("\t\t {}% requires observations at {} <= {:5.2f}\n".format(
            kasen_fraction, filter_list[0], apparent_mag))
    except:
    ...
    try:
        junk, junk, ligo_dist = hp2np.hp2np(
            skymap, degrade=resolution, field=1)
        junk, junk, ligo_dist_sig = hp2np.hp2np(
            skymap, degrade=resolution, field=2)
        junk, junk, ligo_dist_norm = hp2np.hp2np(
            skymap, degrade=resolution, field=3)
...
# I believe as of Jan 2020, n_slots is related primarily to slotDuration and the length of the night
    ...
    print("\t tonight is {:.2f} hours long with {} slots".format(
        hoursPerNight, n_slots))
    ...
    print("=============>>>>  observing")
    if start_slot != -1 or do_nslots != -1:
        print("\t tonight we will use {} slots starting at {}".format(
            do_nslots, start_slot))
    ...
    writeObservingRecord(hoursObserving,   data_dir,
                         gw_map_trigger, gw_map_control, gw_map_strategy)
    ...
    pickle.dump(hoursObserving, open("{}-hoursObserving.pickle".format(trigger_id), "wb"))
    ...
    if ra.size == 1 and prob.sum() == 0:
        ...
        try:
            sum_ligo_prob = how_well_did_we_do(
                gw_map_trigger, gw_map_strategy, gw_map_control)
        except:
            print('\t ZERO PROBABILITY observed!')
            sum_ligo_prob = 0.
...
def make_jsons(gw_map_trigger, gw_map_strategy, gw_map_control, gw_map_results):
    ...
    ra, dec, id, prob, mjd, slotNum, dist = obsSlots.readObservingRecord(
        trigger_id, data_dir)
    ...
    for tiling in tiling_list[1:]:
        if tiling == 0:
            ra_shift, dec_shift = 0.00, 0.00
        if tiling == 1:
            ra_shift, dec_shift = 0.02, 0.00
        if tiling == 2:
            ra_shift, dec_shift = 0.00, 0.02
        if tiling == 3:
            ra_shift, dec_shift = -0.02, 0.00
        if tiling == 4:
            ra_shift, dec_shift = 0.00, -0.02
        print("\t tiling {} ra,dec shift by {},{}".format(
            tiling, ra_shift, dec_shift))
        print("list of jsons, rashift, decshift, tiling",
              list_of_jsons, ra_shift, dec_shift, tiling)
        # setup_tilings(
        #     list_of_jsons,
    ...
    for file in list_of_jsons:
        ...
        shutil.copyfile(dir+"/"+file, dir +
                        "/json/tiling_{}_".format(tiling_list[0]) + file)
...
def makeObservingPlots(gw_map_trigger, gw_map_strategy, gw_map_control, gw_map_results, allSky=True):
    ...
    print("================ >>>>>>>>>>>>>>>>>>>>> =================== ")
    print("We're going to do {} slots with best slot={}".format(
        made_maps_list.size, best_slot))
    ...
    string = "$(ls -v {}observingPlot*)  {}_{}animate.gif".format(data_dir +
                                                                  '/'+trigger_id+'-'+label, data_dir+'/'+trigger_id, label)
    print(string)
...
def how_well_did_we_do(gw_map_trigger, gw_map_strategy, gw_map_control):
    ...
    name = os.path.join(data_dir, str(trigger_id) +
                        "-ra-dec-id-prob-mjd-slot-dist.txt")
    raH, decH = np.genfromtxt(name, unpack=True, usecols=(0, 1))
    treedata = decam2hp.buildtree(ra, dec, resolution, recompute=True)
    tree = treedata[2]
    sum = decam2hp.hexalateMapWithoutOverlap(
        ra, dec, ligo, tree, raH, decH, camera, verbose=False)
    print("\nTotal Ligo probability covered by hexes observed: {:.3f}%".format(
        sum.sum()*100.))
...
def old_reuse_results(gw_map_trigger, gw_map_strategy, gw_map_control, gw_map_results, get_slots=False):
    ...
    if get_slots:
        hoursObserving = pickle.load(
            open("{}-hoursObserving.pickle".format(data_dir+'/'+trigger_id), "r"))
        ra, dec, id, prob, mjd, slotNum, islot = obsSlots.slotsObservingToNpArrays(
            hoursObserving)
...
def economics(simNumber, best_slot, mapDirectory,
    ...
    max_area = 11734.
    area, cum_prob = cumul.area(
        ra, dec, obsProb, p_gw, nsides, max_area=max_area)
    ...
    if fraction_of_sims_better_than_this_trigger < 1./N_max:
        area, cum_prob = cumul.area(
            ra, dec, obsProb, prob, nsides, max_area=max_area)
        if area > area_left:
            print("\t maxing out area: \t {:.3f} -> ".format(cum_prob)),
            cum_prob = cumul.probability_covered(
                ra, dec, obsProb, area_left, nsides, max_area=max_area)
...
def turnObservingRecordIntoJSONs(ra, dec, id, prob, mjd, slotNumbers,
                                trigger_id, exposure_list, filter_list,
                                tiling_list, trigger_type, skymap,
                                mapDirectory, propid):
    seqtot = ra.size
    seqzero, seqnum = 0, 0
...
def utcFromMjd(mjd, alt=True):
    ...
    time = "UT-{}-{:02d}-{:02d} {:02d}:{:02d}:00".format(
        year, month, day, hour, minute)
    if alt:
        time = "UT-{}-{:02d}-{:02d}_{:02d}:{:02d}:00".format(
            year, month, day, hour, minute)
    return time
...
def jsonFromRaDecFile(radecfile, nslots, slotZero, hexesPerSlot, simNumber,
                        mjdList, trigger_id, trigger_type, data_dir):
    
    """
    propid not defined here.
    """
    ra, dec = np.genfromtxt(radecfile, unpack=True, usecols=(
        0, 1), comments="#", propid="propid")
...
def cumulPlot(trigger_id, data_dir):
    ...
    new_x, new_y, new_mjd, new_hour = np.array(
    ...
    for q in [0.25, 0.5, 0.75]:
        ax2.plot([interp(q), interp(q)], [0, new_cy.max()],
                 alpha=0.3, c="r", ls="dotted")
    ...
    if len(mjd_h) < 2:
        # added if to make interp1d inputs the same
        mjd_h = np.append(mjd_h, mjd_h[0])
...
def equalAreaPlot(figure, slot, simNumber, data_dir, title=""):
    ...
    name = os.path.join(data_dir, str(simNumber) +
                        "-"+str(slot)+"-maglim-eq.png")
    print(name)
    plt.clf()
    mcplot.plot(ra, dec, maglim, vmin=17)
    plt.plot(desx, desy, color="w")
    plt.xlabel("RA")
    plt.ylabel("Dec")
    plt.title(title)
    ...
    name = os.path.join(data_dir, str(simNumber)+"-" +
                        str(slot)+"-probXligo-eq.png")
    print(name)
    plt.clf()
    mcplot.plot(ra, dec, prob*ligo)
    plt.plot(desx, desy, color="w")
    plt.xlabel("RA")
    plt.ylabel("Dec")
    plt.title(title)
...
def observingPlot(figure, simNumber, slot, data_dir, nslots, n_hexes, camera,
    ...
    if n_hexes > 0:
        ...
        ra, dec, id, prob, mjd, slotNumbers, dist = obsSlots.readObservingRecord(
            simNumber, data_dir)
        ix = slotNumbers == slot
        if np.any(ix):
            the_mjd = mjd[ix][0]
...
def writeObservingRecord(slotsObserving, data_dir, gw_map_trigger, gw_map_control, gw_map_strategy):
    ...
    if max_number_of_hexes_to_do < 10000:
        ...
        print("\t ========================================")
        print("\t number of hexes kept for jsons = {}".format(
