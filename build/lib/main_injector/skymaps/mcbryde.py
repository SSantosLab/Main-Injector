...
def mcbryde(ra, dec, southUp=0, test=0, alpha=0, beta=0, isLine=False):
    ...
    if southUp:
        y = -y
    if type(x) is np.array and len(x) == 1:
        x = x[0]
        y = y[0]
    ...
    if test:
        ...
        q1 = 0.31246*R*ra*(1 + 2 * np.cos(psi*2)/np.cos(psi))
        r1 = 1.87476*R*np.sin(psi)
        print("alt x,y for {:d} {:d}= {:8.3f} {:8.3f}".format(
            rao, alpha, round(q1, 3), round(r1, 3)))
        ...
        print("alt x,y for {:d} {:d}= {:8.3f} {:8.3f}".format(
            rao, alpha, round(q1, 3), round(r1, 3)))
        print("cal x,y for {:d} {:d}= {:8.3f} {:8.3f}".format(
            rai, deci, round(x, 3), round(y, 3)))
...
def readMT():
    ...
    psispline = interp1d(np.array(dec, dtype='float'), np.array(
        psi, dtype='float'), fill_value="extrapolate", bounds_error=False)
