...
def usage():
    print(
        'Usage: cumulative_plots.py [-d <path_to_files>] [-p <path_for_plots> ] -e <event_id> -f <file 1 file 2 ... file n>')
...
i = dict(list(zip(events, range(len(events)))))
...
for event in events:
    ...
    if i[event] == i[this_event]:
        plt.plot(cumprobs, color='blue',
                 label='Event #: '+event, linewidth=3.0)
    elif sims_labeled:
