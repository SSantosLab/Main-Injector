...
#count, seqid, seqnum, seqtot,note,comment
            ...
            tdec = tdec+delDec
            tra = tra + delRa/np.cos(tdec*2*np.pi/360.)
            comment = "DESGW: LIGO event {}: {} of {}".format(
                seqid, seqnum, seqtot)
            ...
            if tdec < 0:
                signDec = "-"
            object = "DESGW: LIGO event {}: {} of {}".format(
                seqid, seqnum, seqtot)
...
def test():
    ...
    import json
    mjdList = ("UTC-2015-9-20-6:29:00", "UTC-2015-9-20-6:59:00", "UTC-2015-9-20-7:29:00",
               "UTC-2015-9-20-7:59:00", "UTC-2015-9-20-8:29:00", "UTC-2015-9-20-8:59:00")
    ...
    j = open(dir+f1)
    data6 = json.load(j)
    x = [(e["RA"], e["dec"]) for e in data6]
    ra6, dec6 = list(zip(*x))
    j.close()
    j = open(dir+f2)
    data7 = json.load(j)
    x = [(e["RA"], e["dec"]) for e in data7]
    ra7, dec7 = list(zip(*x))
    j.close()
    j = open(dir+f3)
    data8 = json.load(j)
    x = [(e["RA"], e["dec"]) for e in data8]
    ra8, dec8 = list(zip(*x))
    j.close()
    j = open(dir+f4)
    data9 = json.load(j)
    x = [(e["RA"], e["dec"]) for e in data9]
    ra9, dec9 = list(zip(*x))
    j.close()
    j = open(dir+f5)
    data10 = json.load(j)
    x = [(e["RA"], e["dec"]) for e in data10]
    ra10, dec10 = list(zip(*x))
    j.close()
    j = open(dir+f6)
    data11 = json.load(j)
    x = [(e["RA"], e["dec"]) for e in data11]
    ra11, dec11 = list(zip(*x))
    j.close()

    plt.clf()
    plt.scatter(ra6, dec6, c="cyan")
