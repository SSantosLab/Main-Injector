...
def makeini(season, ligoid, triggerid, propid, recycler_mjd, bands="all"):

    config.add_section('general')
    config.set('general', 'season', season) #416/417 
    config.set('general', 'ligoid', ligoid) #GW170814, possibly in yaml?
    config.set('general', 'triggerid', triggerid) #G297595 in yaml
    config.set('general', 'propid', propid) #2017B-0110 not in yaml
    config.set('general', 'triggermjd', recycler_mjd) #57979.437 in yaml
    ...
    config.set('general', 'outdir', '/data/des41.a/data/desgw/postproc/'+str(season)+'/outputs')
...
#exposures_ZDtest.list ; # exposures_GW170814_teff_cut.list ; #if you have this, store it in indir indicated above
    config.set('general', 'bands', bands) # all if using all bands, list or single band if making selection (ex: i,z or i)
    ...
    with open('postproc.ini', 'wb') as configfile:
        config.write(configfile)
    configfile.close()

    return configfile

#makePostProcini(season="417", ligoid="GW170817", triggerid="G297595", propid="2017B-0110", recycler_mjd="57979.437", bands="all")
