from main_injector.utils import decam2hp
import numpy as np
license = """
...
def hexalateNHexes(obs, sm, nHexes, allskyDesHexes):
    raHexen, decHexen, idHexen, hexVals, rank = cutAndHexalate(
        obs, sm, allskyDesHexes)
...
def cutAndHexalateOnRaDec(obs, sm, raHexen, decHexen,
idHexen, tree, camera, cutProbs=False):
    ...
    if not cutProbs:
        hexVals[ix2] = decam2hp.hexalateMap(obsRa, obsDec, probabilities,
                                            tree, raHexen[ix2], decHexen[ix2],
                                            camera, verbose=False)
    else:
        hexVals[ix2] = decam2hp.hexalateMapWithoutOverlap(obsRa,
                                                        obsDec,
                                                        probabilities,
                                                        tree,
                                                        raHexen[ix2],
    ...
    # sort from large to small by flipping natural argsort order
    rank = rank[::-1]
