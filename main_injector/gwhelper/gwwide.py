...
from main_injector.sky.telescope import blancoHorizonLimits as load_blanco_limits
...
def gwwide(wide_queue, gw_queue, start_time, dt, sort, camera="decam"):
    ...
    start_time = Time(start_time, format='mjd', scale='utc',
                      location=(obs_longitude, obs_latitude))
    start_time.format = "iso"
    start_time = start_time.value

    fixed_queue = [fix_obs(gw_obs, wide_queue, start_time, dt, camera)
                   for gw_obs in gw_queue]
    print(fixed_queue)
    if sort:
...
def fix_obs(gw_obs, all_filter_wide_queue, start_time, dt, camera):
    ...
    gw_obs['earlyTime'] = rising_iso8601(
        gw_obs['RA'], gw_obs['dec'], start_time, dt, camera)
    gw_obs['lateTime'] = setting_iso8601(
        gw_obs['RA'], gw_obs['dec'], start_time, dt, camera)
...
def file_gwwide(gw_fnames, wide_fname, start_time, dt, fixed_fname, sort):
    ...
    logging.info("Loaded %d wide survey entries from %s",
                 len(wide_queue), wide_fname)
    ...
    for gw_fname in gw_fnames:
        ...
        logging.info("Loaded %d GW followup entries from %s",
                     len(new_gw_queue), gw_fname)
    ...
    logging.info("Saved %d GW followup entries in %s",
                 len(fixed_queue), fixed_fname)
...
def setting_ha(decl):
    ...
    # Eric had -88; I do not know why.
    setting_ixs = (ha_limits < 0) & (decl_limits >= -90)
...
def rising_ha(decl):
    ...
    decl_limits = np.degrees(rad_decl_limits)
    # Eric had -88; I do not know why.
    rising_ixs = (ha_limits < 0) & (decl_limits >= -90)
...
def main():
    ...
    parser.add_argument(
        'fixed_script', help='Filename we write fixed script to')
    parser.add_argument('gw_scripts', nargs='+',
                        help='Filename of GW obs script')
    parser.add_argument(
        '-t', '--start_time', help='Start time (ISO 8601)', default=current_timestring)
    parser.add_argument(
        '-d', '--dt', help='time buffer before setting (seconds)', type=float, default=300)
