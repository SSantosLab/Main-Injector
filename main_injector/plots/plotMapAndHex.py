...
from main_injector.skymaps import mcbryde
from main_injector.utils import hp2np, rotate, decam2hp
from main_injector.skymaps.inside_footprint import get_footprint, get_footprint_RaDec
...
def mapAndHex(figure, simNumber, slot, data_dir, nslots, hexRa, hexDec, camera,
        ...
        scale=1., moonRa=-999, moonDec=-999, moonIllumination=0):
    ...
    if doStars :
        ...
        stars = hp.read_map(os.environ["DESGW_CATALOGS"])
...
def coreMapAndHex(figure, hexRa, hexDec, raMap, decMap, camera, map, 
    ...
    if doHexes :
        ...
        #ix =np.invert( inside_footprint.insideFootprint(hexRa, hexDec))
