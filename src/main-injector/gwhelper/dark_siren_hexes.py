import numpy as np
import healpy as hp
from utils import hp2np as hp2np
import matplotlib.pyplot as plt

# make the json files
# Jan 4
# reload(dsh); dsh.get_tiling_ra_dec(ra_hex, decHexen, sumProb, nu, ng, nr, ni, nz, slice=24, n=3)
# Feb 2
# reload(dsh); dsh.get_tiling_ra_dec(ra_hex, decHexen, sumProb, nu, ng, nr, ni, nz, slice=30, n=4)
# reload(dsh); dsh.get_tiling_ra_dec(ra_hex, decHexen, sumProb, nu, ng, nr, ni, nz, slice=62, n=3)
def get_tiling_ra_dec(ra_hex, decHexen, sumProb, nu, ng, nr, ni, nz, slice=24, n=3) :
...
def examine (ra_hex, decHexen, sumProb, nu, ng, nr, ni, nz, slice=30, n=4) :
    ix=np.argsort(sumProb)[::-1]
    ...
    for i in range(0,slice): 
        print("{:3d} {:5.1f}%   {:3.0f} {:3.0f} {:3.0f} {:3.0f} {:3.0f}   {:10.4f} {:10.4f}".format(
            ...
            nu[ix[i]], ng[ix[i]], nr[ix[i]], ni[ix[i]],nz[ix[i]], ra_hex[ix[i]], decHexen[ix[i]]))
