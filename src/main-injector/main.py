...
from hex import hex_observations
import subprocess
import datetime
import yaml
...
import gw_helper.gw_map_configure as gw_map_configure
import matplotlib.pyplot as plt


class Event:
    def __init__(self, skymap_filename: str, master_dir: str, trigger_id: str, mjd: float, config: str, official, hasrem):
        '''
        event:
...
# desgw-maps.
    ...
    def mapMaker(self, trigger_id, skymap, config, hasrem, snarf_mi_maps=False, start_slot=-1, do_nslots=-1,  mi_map_dir="./"):
        #skymap_filename, master_dir, trigger_id, mjd, config, official, hasrem
        import os
        import yaml
        import observations
...
#        if do_make_maps:
    ...
    def updateWebpage(self, real_or_sim):
        ...
        os.system('scp -r ' + GW_website_dir_t +
                  self.trigger_id + ' ' + desweb_t)
