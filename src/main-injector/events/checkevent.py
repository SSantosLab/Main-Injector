...
import getopt
import logging
from utils import send_texts_and_emails
from events import checkevent_config as config
import subprocess
import healpy as hp
...
sys.path.append(
    '/data/des41.a/data/desgw/osgsetup/eeups/fnaleups/Linux64/pygcn/pygcn-0.1.10/')
...
#              for elem in root.iterfind('.//Param')}
    ...
    try:
        event_params['ChirpMass'] = str(root.find("./What/Param[@name='ChirpMass']").attrib['value'])+' '+str(
    ...
    try:
        event_params['CentralFreq'] = str(root.find("./What/Param[@name='CentralFreq']").attrib['value'])+' '+str(
    ...
    try:
        sendFirstTriggerEmail(trigger_id, event_params['FAR'], mapname=skymap_url.split(
            '/')[-1].split('.')[0], retraction=alerttype, event_params=event_params)
    except:
        sendFirstTriggerEmail(
            trigger_id, event_params['FAR'], retraction=alerttype)
...
document.getElementById("datetime").innerHTML = dt.toUTCString();\
    ...
    os.system(
        'scp imalive.html codemanager@desweb.fnal.gov:/des_web/www/html/desgw/')
...
if __name__ == "__main__":
    ...
    if config.mode.lower() == 'mdc':
        ...
        KeyError(
            'checkevent_config.py Mode must be set to either test or observation.\nExiting...')
